        ...
        if (err) {
          ...
          if (true==result) {
            if ('红包'==$.prizeName) {
              ...
              console.log(`获得满${quota}减${discount}优惠券（指定店铺可用）`)
            }else if('全品类(特例商品除外)'==$.prizeName){
              console.log(`获得满${quota}减${discount}全品类(特例商品除外)`)
            }else{
              console.log(`获得${$.prizeName}`)
            }
          }
        }
      } catch (e) {
...
function jdbalance() {
  return new Promise((resolve) => {
    $.get(balanceUrl(), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
...
function balanceUrl() {
  return {
    url: `https://wq.jd.com/user/info/QueryUserRedEnvelopes?channel=3&type=1&page=0&pageSize=100&orgFlag=JD_PinGou_New&expiredRedFlag=1&_=${new Date().getTime()}&sceneval=2&g_login_type=1&g_ty=ls`,
    headers: {
      "accept": "*/*",
